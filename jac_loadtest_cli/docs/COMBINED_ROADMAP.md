...
- [ ] Auto-detect GraphQL endpoints in imported HAR; render with dedicated GraphQL UI — the detection itself is now done CLI-side (`HarEntry.protocol`, see the CLI checklist above); this item is the remaining `cl` rendering work on top of it, not new sv-side detection logic
