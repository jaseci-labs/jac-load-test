...
- [x] Wire `--url`, `--vus`, `--duration`, `--timeout` flags in `cli.jac`
...
- [x] Think time in `engine.jac`: `--think-time none|real` with `--think-time-scale` multiplier; `scaled` mode noted as separate from `real`
- [x] Ramp-up in `engine.jac`: `--ramp-up Ns` staggers VU startup; VU i starts at `(i / vus) * ramp_up_s`
- [x] Config three-layer resolution in `config.jac`: CLI flags → jac.toml `[plugins.scale.loadtest]` → built-in defaults; all toml-resolvable CLI args use `default=None` in `plugin.jac` so argparse defaults can't mask jac.toml values
...
**Notes from implementation:**
- `plugin.jac` arg defaults must be `None` for all toml-resolvable fields; built-in defaults live only in `BUILT_IN_DEFAULTS` in `config.jac` — never duplicated in argparse defaults
...
- [x] `core/har_parser.jac` — `target_url` made optional; microservice mode keeps recorded URLs, topology handles routing
...
- [ ] Exit codes: `0` = completed + all thresholds pass, `1` = threshold failed, `2` = config/tool error — **flags are parsed and stored in `LoadTestConfig` but enforcement logic is not yet wired in `cli.jac`; currently all runs exit `0` unless a config error triggers `sys.exit(2)`**
...
- [ ] RPS cap: `--rps N` via token bucket (`asyncio.Semaphore`) — **`config.rps` is stored but the token-bucket check does not exist in `engine.jac`; the flag is accepted but has no effect**
- [ ] `--think-time scaled` — currently `engine.jac` only checks `config.think_time == "real"`; passing `--think-time scaled` behaves identically to `none` (no delay); needs its own branch so `scaled` can be used without `real` semantics
- [x] `error_type` on `RequestResult`: `None` | `"TIMEOUT"` | `"CONNECTION_REFUSED"` | `"DNS_ERROR"` | `"SSL_ERROR"` | `"SERVER_DISCONNECTED"` | `"CONNECTION_RESET"` — all catch blocks implemented in `engine.jac`
...
- [x] Multi-process VU distribution: `--workers N` flag + `core/process_runner.jac` — splits VUs evenly across N worker processes (each with its own asyncio loop), authenticates all credentials once in the main process before spawning workers, merges samples from all workers into a single `MetricsCollector`; worker count capped at VU count; uses `spawn` context for asyncio compatibility
...
**Exit criterion:** interrupted test at minute 9 of 10 still generates a partial report. CI pipeline `if [ $? -ne 0 ]` correctly detects threshold failures. `jac test tests/integration/` passes.
...
- [x] `--report-out path` flag for JSON/HTML destination (CLI only — already wired in plugin.jac and config.jac)
- [ ] `--debug` flag: per-request lines to stderr — **`config.debug` is stored but never read in `engine.jac` or `cli.jac`; has no effect yet**
...
**Notes from implementation:**
- `render_json()` returns a JSON string; `cli.jac` routes it to stdout (default) or a file when `--report-out` is set
...
- Timeseries data is generated post-run by `MetricsCollector.generate_timeseries(t_start)` in `cli.jac`, which bins all collected `RequestResult` samples into 10-second buckets; there is no in-run snapshot loop
